 ...
 *
 * 각 유형의 actions는 작업평가(JRA)에서 그 유형을 고르면 후보로 뜨는 '현재 조치사항'
 * 목록이다 — 예전에는 jobAssessment.ts의 ACTIONS_BY_CODE에 코드로만 따로 있었는데,
 * 설정 화면에서 위험분류와 함께 고치게 하려고 이 분류표 안으로 옮겼다(설정 →
 * 유해위험요인 분류표에서 유형마다 펼쳐서 추가·삭제한다).
 */
export type HazardType = {
  code: string; // 유해위험유형 번호 (예: "1.4")
  label: string; // 유형 이름 (예: "부딪힘")
  actions: string[]; // 작업평가 '현재 조치사항' 후보 — 없으면 빈 배열(직접 입력만 가능)
};
...
export const HAZARD_FACTORS: HazardFactor[] = [
    ...
    types: [
      {
        code: "3.1",
        label: "유해가스",
        actions: ["작업장소 유해가스 발생 유무 확인", "방독마스크 비치 및 상태 확인", "탈취설비 정상가동 유무 확인"],
      },
      { code: "3.2", label: "고온증기", actions: ["고온증기 배관 접촉방지 유무", "고온증기 누기 상태 확인"] },
      {
        code: "3.3",
        label: "흄",
        actions: [
      ...
      },
      {
        code: "3.4",
        label: "화학물질 접촉·누출",
        actions: ["MSDS 비치 및 경고표시 등 내용 확인 유무", "유해화학물질 누액 상태 확인", "내화학용 보호구 착용 유무"],
      },
      {
        code: "3.5",
        label: "분진",
      ...
      },
      {
        code: "3.9",
        label: "폭발/파열",
        actions: ["방폭형 작업도구 사용 유무", "인화성물질 및 가스 누출 유무", "설비 내부 인화성물질 제거 유무"],
      },
    ],
  },
  {
    ...
    name: "생물학적",
    types: [
      { code: "4.1", label: "병원성 미생물, 바이러스 감염", actions: ["응급의료체계 숙지 여부"] },
      { code: "4.2", label: "알러지", actions: [] },
    ],
  },
  {
    no: "5",
    ...
    types: [
      ...
      { code: "5.3", label: "압력상태", actions: [] },
      {
        code: "5.4",
        label: "소음",
        actions: ["귀마개 등 청력보호구 비치 및 착용 확인", "작업환경측정결과 내 유해인자 소음 유무 확인"],
      },
      { code: "5.5", label: "진동", actions: [] },
      {
        code: "5.6",
        label: "중량물취급",
        actions: ["올바른 중량물 취급자세 숙지 유무", "중량물 무게중심 확인", "중량물 운반대차 상태 확인"],
      },
      { code: "5.7", label: "반복작업", actions: ["구성원 근골격계질환 유무 확인", "충분한 휴게시간 부여"] },
      {
        code: "5.8",
        label: "이상온도·물체 접촉(화상)",
        actions: [
      ...
      { code: "5.9", label: "불안정한 작업자세", actions: ["충분한 작업공간 확보 여부"] },
      {
        code: "5.10",
        label: "부적절 작업도구",
        actions: ["사용 작업도구 외관 상태 점검 유무", "해당 작업에 필요한 적절한 도구 확인"],
      },
    ],
  },
  {
    ...
    name: "작업환경",
    types: [
      { code: "6.1", label: "기후(고온 또는 한랭)", actions: [] },
      {
        code: "6.2",
        label: "조도(조명)",
        actions: [
      ...
      },
      {
        code: "6.3",
        label: "공간 및 통로 미확보",
        actions: ["통로의 부식 등 외관상태 점검 유무", "기계 설비 등 접근이 가능한 통로 확보 유무"],
      },
      { code: "6.4", label: "주변 근로자", actions: [] },
      { code: "6.5", label: "작업시간", actions: [] },
      // 6.6·6.7은 원본 서식에 없는 추가분이다 — 6.1 기후에 뭉뚱그리면 폭염·한랭 각각의
      // 법정 조치사항(체감온도별 휴식 부여 등)을 작업평가에서 나눠 쓸 수 없다
      {
        code: "6.6",
        label: "폭염·온열질환",
        actions: [
  ...
  },
  {
    ...
    name: "기타",
    types: [
      { code: "7.1", label: "정리정돈", actions: ["작업장소 청결상태 확인 유무"] },
      { code: "7.2", label: "기타사항", actions: [] },
      // 7.3은 원본 서식에 없는 추가분이다 — 출퇴근 교통재해 항목이 많아
      // 전부 "기타사항"으로 뭉치면 집계가 무의미해져서 따로 뒀다
      { code: "7.3", label: "교통안전", actions: [] },
    ],
  },
];
