/**
 * 위험성평가 메뉴 — '정기평가'·'작업평가'·'상시평가' 세 탭.
 *
 * 셋은 주기가 다르다. 정기평가(위험성평가표)는 **공정 단위로 한 해 한 장**을, 작업평가는
 * **작업 한 건마다 한 장**을, 상시평가(TBM·일일교육)는 **작업 당일마다 한 장**을 쌓는다.
 * 필드가 거의 겹치지 않아 저장소와 화면을 따로 두되, 현장에서는 모두 '위험성평가'라
 * 한 메뉴 안에 탭으로 나란히 뒀다.
 *
...
export function AssessmentTabs({ openId, onOpen }: { openId: string | null; onOpen: (id: string | null) => void }) {
  ...
  React.useEffect(() => {
    ...
    else if (routines.some((v) => v.id === openId)) setTab("routine");
    else if (assessments.some((v) => v.id === openId)) setTab("regular");
  }, [openId, assessments, jobAssessments, routines]);

  const detailOpen =
    !!openId &&
    (assessments.some((v) => v.id === openId) ||
      jobAssessments.some((v) => v.id === openId) ||
      routines.some((v) => v.id === openId));

  const TABS: { key: Tab; label: string; count: number }[] = [
    { key: "regular", label: "정기평가", count: assessments.length },
    { key: "job", label: "작업평가", count: jobAssessments.length },
    { key: "routine", label: "상시평가", count: routines.length },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "regular" && <AssessmentsPage openId={openId} onOpen={onOpen} />}
      {tab === "job" && <JobAssessmentsPage openId={openId} onOpen={onOpen} />}
      {tab === "routine" && <RoutinesPage openId={openId} onOpen={onOpen} />}
    </div>
  );
}
