 ...
 * 이력 관리 · 회의·교육 실시서 목록.
 *
 * 서식이 두 종류(사전 교육·회의 / 결과 교육)라 **발행 버튼도 두 개**다.
...
import { headcountOf, signedCount, TRAINING_KINDS, type Training, type TrainingKind } from "@/lib/types";
import { cn } from "@/lib/utils";
import { useStore } from "@/store";

/** 구분별 색 — 두 서식을 목록에서 한눈에 가르기 위한 것 */
...
export function TrainingsPage({ openId, onOpen }: { openId: string | null; onOpen: (id: string | null) => void }) {
  ...
  const sorted = [...trainings]
    ...
    .filter((v) => {
      ...
      return [v.instructor, v.place, v.eduContent, v.meetContent].some((t) => (t || "").toLowerCase().includes(query));
    });
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="mt-1 text-sm text-muted-foreground">
          실시한 교육·회의를 서식 그대로 남깁니다. 참석자는 게스트로 들어와 자기 이름 옆에 직접 서명할 수 있습니다.
        </p>
        {/* 발행 버튼 두 개 — 서식마다 들어가는 칸이 달라 처음부터 종류를 고르게 한다 */}
        <div className="flex flex-wrap gap-2">
          {TRAINING_KINDS.map((kind) => (
            <Button
              ...
              title={canEdit ? `${kind} 실시서를 새로 발행합니다` : "발행은 관리자만 할 수 있습니다"}
            >
              {kind} 발행
            </Button>
          ))}
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <EmptyState icon={<GraduationCap className="size-6 text-muted-foreground" />}>
              발행된 실시서가 없습니다. 위의 발행 버튼으로 시작하세요.
            </EmptyState>
          ) : sorted.length === 0 ? (
            <EmptyState icon={<GraduationCap className="size-6 text-muted-foreground" />}>
              조건에 맞는 실시서가 없습니다.
              ...
              <Table className="[&_:is(th,td)]:px-4">
                <THead>
                  <TR>
                    ...
                    <TH className="w-32">교육강사</TH>
                    <TH className="w-24 text-center">교육인원</TH>
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}>
        <DialogHeader>
          <DialogTitle>이 실시서를 삭제할까요?</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-muted-foreground">
          {deleteTarget?.date || "-"} · {deleteTarget?.kind} · {deleteTarget?.instructor || "강사 미입력"}
          <br />
