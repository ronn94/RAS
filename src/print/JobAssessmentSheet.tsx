...
/**
 * 화면에서 잰 높이와 실제 인쇄 높이 사이의 여유.
 *
 * 측정(1단계)과 검증(3단계) 둘 다 화면 렌더링(getBoundingClientRect)으로 재는데, 이
 * 문서는 1쪽 상단(개요·작업전준비)이 유난히 커서(70mm대) 남는 여유가 얼마 없다 —
 * 그 상태에서 화면과 인쇄 사이에 실제로 존재하는 몇 mm 오차만으로도 "화면에서는
 * 맞았는데 인쇄에서는 넘치는" 일이 생긴다. 넘치면 우리 페이지 나누기가 모르는 채로
 * Chrome이 그 표를 한 번 더 쪼개면서, 실제 내용보다 긴 세로 테두리가 찍히는 부작용까지
 * 함께 난다(실제로 겪은 문제) — 그래서 다른 서식(3mm)보다 넉넉하게 잡는다.
 */
const SAFETY_MM = 25;
